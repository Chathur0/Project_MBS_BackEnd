...
router.put("/updateRoom/:id", (req, res) => {
  ...
  const {
    roomNumber, roomType, area, capacityAdult, capacityChild,
    pricePerDay, description, view, headlines, technologies,
    services, beds, baths
  } = req.body;
  ...
  `;
  const values = [
    roomNumber, roomType, area, pricePerDay, description, view, capacity,
    JSON.stringify(beds), JSON.stringify(technologies), JSON.stringify(baths),
    JSON.stringify(headlines), JSON.stringify(services), roomId
  ];
